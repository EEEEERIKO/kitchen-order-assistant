...
export function ProductForm({ onAddProduct, onProductAdded, enableQuantityMode, onToggleQuantityMode }: ProductFormProps) {
  ...
  const handleDuplicateDetected = (existingProductName: string) => {
    setDuplicateMessage(`✓ ${t.form.quantityIncreasedMessage}: ${existingProductName}`)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Validation
    if (!productName.trim()) {
      setError(t.form.emptyProductError)
      return
    }

    if (enableQuantityMode && quantity < 0) {
      setError(t.form.negativeQuantityError)
      return
    }
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-sm font-semibold transition-colors ${enableQuantityMode ? 'text-primary' : 'text-gray-500 dark:text-gray-400'}`}>
              {enableQuantityMode ? t.form.quantitiesActive : t.form.onlyProducts}
            </span>
          </div>
          <button
            type="button"
            ...
            aria-label={t.form.toggleQuantitiesLabel}
            title={enableQuantityMode ? t.form.disableQuantitiesTitle : t.form.enableQuantitiesTitle}
          >
            <span
